...
public class XpubFull {
	...
	public XpubFull(String address, int txCount, long totalReceived, long totalSent, long finalBalance, int changeIndex,
			int accountIndex, int gapLimit, List<Transaction> txs) {
	...
	public XpubFull(JsonObject json) {
		this(json.has("address") ? json.get("address").getAsString() : "",
				...
				json.has("gap_limit") ? json.get("gap_limit").getAsInt() : 0, null);
