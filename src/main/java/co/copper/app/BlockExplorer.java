...
import co.copper.app.entity.address.Address;
import co.copper.app.entity.address.Balance;
import co.copper.app.entity.address.Block;
import co.copper.app.entity.address.FilterType;
import co.copper.app.entity.address.LatestBlock;
import co.copper.app.entity.address.MultiAddress;
import co.copper.app.entity.address.SimpleBlock;
import co.copper.app.entity.address.Transaction;
import co.copper.app.entity.address.UnspentOutput;
import co.copper.app.entity.address.XpubFull;
 ...
 * https://blockchain.info/api/blockchain_api. It can be used to query the block
 * chain, fetch block, transaction and address data, get unspent outputs for an
 * address etc.
 */
public class BlockExplorer {
	 ...
	 *                use null to indicate default
	 * @return An instance of the {@link Address} class
	 * @throws APIException If the server returns an error
	 */
	public Address getAddress(String address, FilterType filter, Integer limit, Integer offset)
	...
	/**
	 * Gets a list of blocks at the specified height. Normally, only one block will
	 * be returned, but in case of a chain fork, multiple blocks may be present.
	 *
	 ...
	 * @param confirms    an integer for minimum confirms of the outputs, use null
	 *                    to indicate default
	 * @param limit       an integer to limit number of transactions to display, use
	 *                    null to indicate default
	 ...
	 * @throws APIException If the server returns an error
	 */
	public List<UnspentOutput> getUnspentOutputs(List<String> addressList, Integer confirms, Integer limit)
			throws APIException, IOException {
		...
		try {
			...
			// The server endpoint will return error if no output, however this should be a
			// valid situation so we return an empty result.
	 ...
	 * @param filter      the filter for transactions selection, use null to
	 *                    indicate default
	 * @return a map of (address, {@link Balance})
	 */
	public Map<String, Balance> getBalance(List<String> addressList, FilterType filter)
			throws APIException, IOException {
		Map<String, String> params = buildBasicRequest();
		String pipedAddresses = StringUtils.join(addressList, "|");
		params.put("active", pipedAddresses);
	 ...
	 * @throws APIException If the server returns an error
	 */
	public MultiAddress getMultiAddress(List<String> addressList, FilterType filter, Integer limit, Integer offset)
			throws APIException, IOException {
		Map<String, String> params = buildBasicRequest();
		String pipedAddresses = StringUtils.join(addressList, "|");
		params.put("active", pipedAddresses);
	...
	/**
	 * Returns xpub summary on a xpub provided, with its overall balance and its
	 * transactions.
	 *
	 * @param xpub   a xpub address
	 * @param filter the filter for transactions selection, use null to indicate
	 *               default
	 * @param limit  an integer to limit number of transactions to display, use null
	 *               to indicate default
	 * @param offset an integer to set number of transactions to skip when fetch
